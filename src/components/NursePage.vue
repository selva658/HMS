<template>
<h1>Nurse</h1>
 <div v-for="doctor in doctors" :key="doctor.id"><p>{{doctor.name}}</p><router-link :to="'/nurse/'+doctor.id"><button-slot>View Details</button-slot></router-link></div>
...
export default{
    ...
    methods:{
        totalDoctors(){
            fetch("http://localhost:3000/signupDetailsNurse").then((res)=>res.json()).then((res)=>{
                this.doctors=res
                })
        },
         add(){
